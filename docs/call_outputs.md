...
| Unmasked Indel Coverage | `sum(unmasked_coverage_indel_cat)` | Indel coverage before noise masking, summed across all indel opportunity categories. Used to report the pre-masking (unmasked) indel burden in `_indel_burden.txt`. The effective (post-masking) indel coverage is no longer reported here — see `_indel_burden.txt`'s `Genome coverage` field instead. |
