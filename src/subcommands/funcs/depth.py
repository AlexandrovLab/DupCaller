...
def extractDepthSnv(bam, chrom, pos, ref, alt, params,minbq=1):
    ...
    for pileupcolumn in bam.pileup(chrom, pos - 1, pos, min_base_quality=minbq,truncated=True):
...
def extractDepthIndel(bam, chrom, pos, ref, alt, params,minbq=1):
    ...
    otherIndelCount = 0
    processed_read_names = dict()
    for pileupcolumn in bam.pileup(chrom, pos - 1, pos, min_base_quality=minbq,truncate=True):
        if pileupcolumn.pos == pos - 1:
            for pileupread in pileupcolumn.pileups:
                if (
                    pileupread.is_del
...
def extractDepthRegion(bam, chrom, start, end, params):
    ...
    for line in pysam.mpileup("-Q","30","-q",f"{mapq}","-r",f"{chrom}:{start}-{end}",bam).split("\n"):
        if line:
            try:
...
def detectOverlapDiscord(bam, chrom, pos, ref, alt, params,bc1,bc2):
    for pileupcolumn in bam.pileup(chrom, pos - 1, pos, min_base_quality=0,truncated=True,stepper="samtools"):
        if pileupcolumn.pos == pos - 1:
            for pileupread in pileupcolumn.pileups:
                if (
                    pileupread.is_refskip
                ...
                if ((read_bc1 == bc1 and read_bc2 == bc1) or (read_bc1 == bc2 and read_bc2 == bc1)) and qual == 0:
                    return True
    return False