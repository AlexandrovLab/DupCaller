...
def splitBamRegions(bams,num,contigs,step):
    ...
    for bam in bams:
        ...
        for c,contig in enumerate(contigs_sorted):
            print(f"......Splitting bam regions into roughly equal chunks. Counting reads in {contig} of {bam}......")
            contig_len = contig_lens[c]
            current_start = 0
            while current_window + num <= window_nums_cumulative[c]:
                current_arguments = [(bam,contig,current_start + k*step,current_start + k*step + step) for k in range(num)]
                pool = Pool()
                ...
                current_window += num
            current_arguments = [(bam,contig,current_start + k*step,current_start + k*step + step) for k in range(window_nums_cumulative[c]-current_window)]
            pool = Pool()
            counts = np.array(pool.starmap(bamReadCount, current_arguments))
    ...
    cut_inds = np.searchsorted(total_reads_by_windows_cumulative,np.arange(1,num)*chunkSize,)
    cut_contigs = np.searchsorted(window_nums_cumulative,cut_inds)
    cut_pos = (cut_inds - np.concatenate([[0],window_nums_cumulative])[cut_contigs])*step
