...
def estimate_96(trinuc_cov_by_rf,trinuc_mut_by_rf,ref_trinuc,n):
    ...
    for nn in range(nmin.max()):
        ...
        mutnum_uncorrected = trinuc_rate[:,nn].dot(trinuc_mut_cov_by_rf[:,nmin>=nn].sum(axis=1))
        burden_uncorrected[nn] = mutnum_uncorrected/(trinuc_cov_by_rf[:,nmin>=nn].sum(axis=1).sum())
    ...
    return hap_trinuc[:,2],burden[2],burden_lb[2],burden_ub[2],trinuc_rate[:,2],burden_uncorrected[2],burden_uncorrected_lb[2],burden_uncorrected_ub[2]

    
def do_estimate(args):
    ref_trinuc = calculate_ref_trinuc(args)
    prefix = args.prefix
    trinuc_by_rf = pd.read_csv(prefix+"/"+prefix+"_trinuc_by_duplex_group.txt",sep='\t',index_col=0)
        ...
        for ref_base in ["C", "T"]:
            for plus_base in ["A", "T", "C", "G"]:
                ...
                for alt_base in alts:
                    trinucSbs2num[minus_base + "[" + ref_base + ">" +  alt_base + "]" + plus_base] = len(num2trinucSbs)
                    num2trinucSbs.append(minus_base + "[" + ref_base + ">" + alt_base + "]"  + plus_base)
    ...
    for rec in vcf.fetch():
        ...
        if ref == "C" or ref == "T":
            ...
            trinuc = revcomp[trinuc_revcomp[2]]+revcomp[trinuc_revcomp[1]]+revcomp[trinuc_revcomp[0]]
    ...
    corrected_trinuc_num, burden, burden_lb, burden_ub, trinuc_rate, uburden, uburden_lb, uburden_ub = estimate_96(trinuc_by_rf_np,trinuc_mut_np,ref_trinuc,trinuc_by_rf.columns)
    ...
    palette = OrderedDict({"C>A":"#03BDEF","C>G":"#010101","C>T":"#E42926","T>A":"#CBCACA",\
                "T>C":"#A2CF63","T>G":"#ECC7C5"})
    palette_list = ["#03BDEF","#010101","#E42926","#CBCACA",\
                "#A2CF63","#ECC7C5"]
    palette_list = np.repeat(np.array(palette_list),16).tolist()
    ax.bar(SBS96_order,[corrected_trinuc_pd.loc[corrected_trinuc_pd.index==m,corrected_trinuc_pd.columns[0]].values[0] for m in SBS96_order],color=palette_list)
    plt.yticks(size = 60)
    ax.set(ylabel=None)
    ax.get_xaxis().get_label().set_visible(False)   
    for nnn,muttype in enumerate(palette.keys()):
        rect = mpatches.Rectangle((-0.5+nnn*16,1),16,0.2,color=palette[muttype],\
            zorder=0,transform=ax.get_xaxis_transform(),clip_on=False)
        ax.add_patch(rect)
        ax.text(-0.5+nnn*16+8,1.065,muttype,color="#FFFFFF",fontsize=100,horizontalalignment="center",\
                transform=ax.get_xaxis_transform(),verticalalignment="center")
    ...
    ax.set_xlabel("Trinucleotide Context",fontsize=100,weight='bold',fontname='Arial')
    ...
    for nn in range(96):
        nnn = math.floor(nn/6)
        mmm = nn - nnn*6
        axs[nnn,mmm].scatter(trinuc_by_rf_np[int(np.floor(nn/3)),:],trinuc_mut_np[nn,:])
